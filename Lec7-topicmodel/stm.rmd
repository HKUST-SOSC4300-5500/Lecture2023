...
>  If you have your documents already converted to term-document matrices you can ingest them using readCorpus(). 
> If you just have raw texts you will want to start with textProcessor().
...
data$treated <- ifelse(gadarian$treatment == 1, "treated", "control")
# data$party = ifelse(data$pid_rep == 0.5, "neutral", ifelse(data$pid_rep < 0.5, "Rep", "Dem"))
```

First, use quanteda package to clean data and construct *document-term* matrix (since we already learned).
`STM` package can also allow you to doi document-term matrix. You can follow the vignette to see how to do that in `STM`
...
fit a topic model with 3 topics
...
legend(0.9, .1, c("Treated", "Control"), lwd = 2, col = c("blue", "red"))
